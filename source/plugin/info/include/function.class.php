...
function get_profile_type_title($profile_type_id){
	return DB::result_first("SELECT profile_type_title FROM ".DB::table('info_profile_type')." WHERE profile_type_id='{$profile_type_id}'");
}

function get_profile_setting($profile_type_id=""){
	if(!empty($profile_type_id)){
		$profile_type_setting_name = fetch_all("info_profile_type_setting"," WHERE profile_type_id='{$profile_type_id}' ORDER BY profile_type_setting_sort ASC ","profile_setting_name");
	}else{
		$profile_type_setting_name = fetch_all("info_profile_type_setting"," ORDER BY profile_type_setting_sort ASC ","profile_setting_name");
	}
	...
	$profile_setting = fetch_all("info_profile_setting"," WHERE profile_setting_name in ('".implode("','",$profile_type_setting_names)."')  ORDER BY substring_index('".implode(",",$profile_type_setting_names)."', profile_setting_name,1)");
...
function get_profile_type_setting($profile_type_id){
	$type_array = brian_fetch_all("info_profile_type_setting"," WHERE profile_type_id='{$profile_type_id}' ");
	return $type_array;
}

function get_profile_type_jiage($profile_type_id){
	$type_array = brian_fetch_all("info_profile_type_setting"," WHERE profile_type_id='{$profile_type_id}' AND profile_type_setting_jiage='1' ",array('first'=>1,'filter'=>'profile_setting_name'));
...
function get_post_profile($post_id){
	...
	if( is_array($post_id)){
		...
		$post_profile = fetch_all('info_post_profile'," WHERE post_id='{$post_id}' ","*");
...
function is_info_admin($groupid=''){
	global $info_config,$_G;
...
function upload_file($file_name , $up_file_dir,$_w='390',$_h='240',$_cut='0'){
	global $_G,$info_lang,$info_config;
	if($_FILES[$file_name]['size']){
		...
		if(!in_array($ftype, $fileType)){
			$msg =$info_lang['upload_msg_error_1'];
		}
		if($_FILES[$file_name]['size']> $maxSize*1024){
			$msg =$info_lang['upload_msg_error_2'];
		}
		if($_FILES[$file_name]['error'] !=0){
			$msg =$info_lang['upload_msg_error_3'];
		}
		
		if($msg){
			showmessage($msg.$info_lang['upload_msg_error_4']);
		}
...
class table_brian_district 
{
	public function fetch_all_by_upid($upid, $order = null, $sort = 'DESC') {
		...
		if($upid !== null) {
			...
			return brian_fetch_all('info_area'," WHERE upid IN ('{$upid}') {$ordersql} ",array('sort'=>'id'));
		}
		return array();
	}
}

function info($addonid='info.plugin'){
...
function info_check($addonid='info();'){
	$string = file_get_contents(DISCUZ_ROOT.'./source/plugin/info/admin_area.inc.php');
...
function brian_changecity($mod){
	global $info_config;
	$area_id_cookie = getcookie("shenlan_area_id");
	if(!isset($_GET['area_id']) && isset($area_id_cookie)){
		$_GET['area_id'] = $area_id_cookie;
	}
	
	$_GET['area_id'] = empty($_GET['area_id']) && !empty($info_config['area_id']) ? $info_config['area_id'] : intval($_GET['area_id']);
	
	if(isset($_GET['area_id'])){
		dsetcookie("shenlan_area_id",intval($_GET['area_id']),999999999);
	}
	
	if(empty($_GET['area_id']) && $mod !== 'ajax'){
		dheader("location:{$info_config['root']}?mod=ajax&op=changecity");
	}
	
	$area_id = isset($_GET['area_id']) && !empty($_GET['area_id']) ? intval($_GET['area_id']) : 0;
	if(!empty($area_id)){
		$area_title = DB::result_first("SELECT area_title FROM ".DB::table('info_area')." WHERE area_id ='{$area_id}'");
	}else{
		$area_title = "{$info_lang['all']}";
	}
	return array($area_id,$area_title);
}

function brian_output(){
	if(!defined('IN_MOBILE')){
		global $_G,$info_config;
		if($info_config['weijingtai']==1){
...
function brian_output_replace($content){
	if(!defined('IN_MOBILE')){
		global $info_config;
		if($info_config['weijingtai']==1){
			$url = str_replace("/","\/",$info_config['root']);
			$p = '/href="'.$url.'?(.*?)"/e';
...
function brian_output_returnstr($str){
	global $info_config;
	if(stripos($str , "info.php") !==false && stripos($str , "php?") == false ){
		$return_str = str_replace("info.php","info",$str);
		return $return_str;
	}elseif(stripos($str,$info_config['root']."?mod=view&post_id=") !== false && stripos($str , "op=") == false   ){
		$return_str = str_replace($info_config['root']."?mod=view&post_id=",$info_config['siteurl']."info-",$str);
		return $return_str;
	}elseif(stripos($str,$info_config['root']."?mod=list&subcat_id=") !== false && substr_count($str,"&") ==1 ){
		$return_str = str_replace($info_config['root']."?mod=list&subcat_id=",$info_config['siteurl']."info-cat-",$str);
		$return_str = str_replace("&ac=","-",$return_str);
		return $return_str;
	}elseif(stripos($str,$info_config['root']."?mod=select") !== false && stripos($str , "cat_id=") == false   ){
		$return_str = str_replace($info_config['root']."?mod=select",$info_config['siteurl']."info-select",$str);
		return $return_str;
	}elseif(stripos($str,$info_config['root']."?mod=select&cat_id=") !== false ){
		$return_str = str_replace($info_config['root']."?mod=select&cat_id=",$info_config['siteurl']."info-select-cat-",$str);
		return $return_str;
	}elseif(stripos($str,$info_config['root']."?mod=index&area_id=") !== false ){
		$return_str = str_replace($info_config['root']."?mod=index&area_id=",$info_config['siteurl']."info-area-",$str);
		return $return_str;
	}else{
		return $str;
	}
