...
function GooUploader(Div,property){
	...
	this.$btn_add=jq("<div class='upload_btn' style='margin-right:4px;'><div class='left'></div><div><b class='add'>"+(property.btn_add_text||"Add")+"</b></div><div class='right'></div><div id='"+this.$div.attr("id")+"_addFile'></div></div>");
	...
	if(this.$multiple){//如果是支持批量上传，则要多定义另两个按钮
		this.$btn_upload=jq("<div class='upload_btn'><div class='left'></div><div><b class='upload'>"+(property.btn_up_text||"Upload")+"</b></div><div class='right'></div></div>");
        this.$btn_cancel=jq("<div class='upload_btn' style='display:none'><div class='left'></div><div><b class='cancel'>"+(property.btn_cancel_text||"Cancel")+"</b></div><div class='right'></div></div>");
		this.$btn_clean=jq("<div class='upload_btn' style='float:right'><div class='left'></div><div><b class='clean'>"+(property.btn_clean_text||"Clean")+"</b></div><div class='right'></div></div>");
    ...
    if(this.$multiple){
      this.$btn_upload.bind("click",{content:this.$content,fileList:this.$fileList,cancel:this.$btn_cancel},function(e){																							 		try{if(!inthis.uploadStartBefore()) return;}catch(e){}
	...
	this.uploadStartBefore=function(){}
	this.uploadAllComplete=function(){}
	this.getSwfUpload=function(){return inthis.$swfUpload}
