...
function getchatpmappenhouse() {
	var users = document.getElementsByName('users[]');
	var appenhouse = '';
	if(users.length) {
		var comma = '';
		for(var i = 0; i < users.length; i++) {
			appenhouse += comma + users[i].value;
			if(!comma) {
				comma = ',';
			}
		}
	...
	if($('username').value) {
		appenhouse = appenhouse ? (appenhouse + ',' + $('username').value) : $('username').value;
	}
	var href = $('a_appenhouse').href + '&memberusername=' + appenhouse;
	showWindow('a_appenhouse', href, 'get', 0);
}
