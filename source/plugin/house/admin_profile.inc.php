...
$house_lang = lang('plugin/'.$identifier);
...
if(submitcheck('add_profile_setting_submit')){
	...
	if(empty($profile_setting['profile_setting_title'])){
		cpmsg("{$house_lang['admin_profile_inc_php_1']}");
	}
...
if(submitcheck('add_profile_type_submit')){
	...
	if(empty($profile_type['profile_type_title'])){
		cpmsg("{$house_lang['admin_profile_inc_php_3']}");
	}
...
if(submitcheck('add_profile_type_setting_submit')){
	...
	if(empty($profile_type_setting['profile_setting_title'])){
		cpmsg("{$house_lang['admin_profile_inc_php_4']}");
	}
...
if(submitcheck('op',1)){
	if($_GET['op']=='profile_type_setting'){
		...
		showtableheader("1. {$house_lang['admin_profile_inc_php_5']} : {$profile_type['profile_type_title']}");
		echo "<tr>";
		echo "<th></th><th>{$house_lang['admin_profile_inc_php_6']}</th><th>{$house_lang['admin_profile_inc_php_7']}</th><th>{$house_lang['sort']}</th><th></th>";
		echo "</tr>";
		...
		foreach($all_profile_type_settging as $profile_type_settging){
			...
			echo "<td><input type='hidden' value='{$profile_type_settging['profile_type_setting_id']}' name='profile_type_setting_id' /> <input type='submit' name='edit_profile_type_submit' value='{$house_lang['admin_profile_inc_php_11']}' class='btn' /> | <a href='".ADMINSCRIPT."?{$cp_url}&del_profile_type_setting={$profile_type_settging['profile_type_setting_id']}' onclick=\"return(confirm('{$house_lang['admin_profile_inc_php_12']}?'))\">{$house_lang['del']}</a></td>";
			echo "</tr>";
			showformfooter();
			$profile_settging_names[] =$profile_type_settging['profile_setting_name'];
		}
		echo "<tr><th colspan='15' class='partition'>1.1{$house_lang['admin_profile_inc_php_5']} : {$profile_type['profile_type_title']} - {$house_lang['admin_profile_inc_php_15']}[{$house_lang['admin_profile_inc_php_16']}]</th></tr>";
		...
		echo "</select></td>";
		echo "<td><select name='profile_type_setting_status'><option value='1'>{$house_lang['yes']}</option><option value='0'>{$house_lang['admin_profile_inc_php_10']}</option></select></td>";
		echo "<td><input name='profile_type_setting_sort' type='text' size='5'></td>";
		echo "<td><input type='submit' name='add_profile_type_setting_submit' class='btn' value='{$house_lang['admin_profile_inc_php_15']}' /></td>";
		echo "</tr>";
		showformfooter();
		showtablefooter();
	}
...
showtableheader("1. {$house_lang['admin_profile_inc_php_20']}");
$all_profile_type = fetch_all("{$identifier}_profile_type"," ORDER BY profile_type_id ASC ");
echo "<tr>";
echo "<th>{$house_lang['admin_profile_inc_php_21']}</th><th>{$house_lang['admin_profile_inc_php_7']}</th><th></th>";
echo "</tr>";
foreach($all_profile_type as $profile_type){
	...
	echo "<td> <input type='submit' value='{$house_lang['admin_profile_inc_php_11']}' class='btn' /> | <a href='".ADMINSCRIPT."?{$cp_url}&del_profile_type_id={$profile_type['profile_type_id']}'  onclick=\"return(confirm('{$house_lang['admin_profile_inc_php_12']}?'))\">{$house_lang['del']}</a></td>";
	echo "</tr>";
}
/*
...
showtableheader("2. {$house_lang['admin_profile_inc_php_32']}");
$all_profile_setting = fetch_all("{$identifier}_profile_setting");

echo "<tr>";
echo "<th style='width:200px;'>&nbsp;</th><th>{$house_lang['admin_profile_inc_php_6']}</th><th>{$house_lang['admin_profile_inc_php_7']}</th><th>{$house_lang['admin_profile_inc_php_35']}</th><th>{$house_lang['admin_profile_inc_php_36']}</th><th>{$house_lang['admin_profile_inc_php_37']}</th><th>{$house_lang['admin_profile_inc_php_38']}</th><th>{$house_lang['admin_profile_inc_php_39']}</th><th>{$house_lang['admin_profile_inc_php_40']}</th><th></th>";
echo "</tr>";
foreach($all_profile_setting as $profile_setting){
	...
	echo ">{$house_lang['admin_profile_inc_php_47']}</option><option value='checkbox'";
	if($profile_setting['profile_setting_formtype']=='checkbox')	echo " selected='selected' ";
	echo ">{$house_lang['admin_profile_inc_php_48']}</option><option value='input'";
	if($profile_setting['profile_setting_formtype']=='input')	echo " selected='selected' ";
	echo ">{$house_lang['admin_profile_inc_php_49']}</option></select></td>";
	...
	echo "<td> <input type='submit' name='edit_profile_setting_submit' value='{$house_lang['admin_profile_inc_php_11']}' class='btn' /> | <a href='".ADMINSCRIPT."?{$cp_url}&del_profile_setting_name={$profile_setting['profile_setting_name']}'  onclick=\"return(confirm('{$house_lang['admin_profile_inc_php_12']}?'))\" >{$house_lang['del']}</a></td>";
	echo "</tr>";
	showformfooter();
}
...
showformheader($self_url);
echo "<tr>";
echo "<td><input type='text' name='profile_setting_name'><div>{$house_lang['admin_profile_inc_php_54']}.<br>{$house_lang['admin_profile_inc_php_55']} : cardtype</div></td>
		<td><input type='text' name='profile_setting_title'><div>{$house_lang['admin_profile_inc_php_56']} : {$house_lang['admin_profile_inc_php_57']}</div></td>
		<td><select name='profile_setting_status'><option value='1'>{$house_lang['yes']}</option><option value='0'>{$house_lang['admin_profile_inc_php_10']}</option></select></td>
		<td><select name='profile_setting_required'><option value='1'>{$house_lang['yes']}</option><option value='0'>{$house_lang['admin_profile_inc_php_10']}</option></select></td>
		<td><select name='profile_setting_allowsearch'><option value='1'>{$house_lang['yes']}</option><option value='0'>{$house_lang['admin_profile_inc_php_10']}</option></select></td>
		<td><select name='profile_setting_formtype'><option value='select'>{$house_lang['admin_profile_inc_php_47']}</option><option value='checkbox'>{$house_lang['admin_profile_inc_php_48']}</option><option value='input'>{$house_lang['admin_profile_inc_php_49']}</option></select></td>
		<td><textarea name='profile_setting_choices'></textarea><p class='tips'>{$house_lang['admin_profile_inc_php_67']}</p></td>
		<td><input name='profile_setting_size' type='text' size='5'></td>
		<td><input name='profile_setting_unit' type='text' size='5'></td>
		<td><input type='submit' name='add_profile_setting_submit' class='btn' value='{$house_lang['admin_profile_inc_php_68']}' /></td>";
echo "</tr>";
showformfooter();
showtablefooter();
