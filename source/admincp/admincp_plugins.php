...
if(!$operation) {
	...
	//var_dump($plugin['modules']);exit;
	$submenuitem[] = array("7.服务信息内容","plugins&operation=config&do=21&identifier=info&pmod=admin_neirong&dcj_action=list",false,1);
	//var_dump($submenuitem);exit;
	if(empty($_GET['pmod'])) {

		if(!submitcheck('editsubmit')) {
			$operation = '';
			shownav('plugin', $plugin['name']);
			//var_dump($plugin['name']);var_dump($submenuitem);exit;
			showsubmenuanchors($plugin['name'], $submenuitem);
			//echo "11111111111";exit;
			//var_dump($pluginvars);exit;
		...
		if(!$modfile)
		{
			$modfile = "./source/plugin/info/admin_neirong.inc.php";
		}
		//echo $modfile;exit;
		//var_dump($plugin['modules']);exit;
		if($modfile) {
